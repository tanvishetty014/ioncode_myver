...
@router.post("/login")
async def login(login_data: LoginDataRequest, db: Session = Depends(get_db)):
    """Login endpoint without JWT token generation"""
    ...
    print("Stored Password Hash:", stored_password_hash)  # Debugging statement
    stored_salt = user.salt
    # print("Stored Salt:", stored_salt)  # Debugging statement
    # Hash the entered password with the stored salt
    entered_password_hash = sha1(f"{stored_salt}{login_data.password}".encode()).hexdigest()
    print("Entered Password Hash:", entered_password_hash)  # Debugging statement
    # Compare the hashes
...
async def staff_login(login_data: LoginDataRequest, db: Session = Depends(get_db)):
    """Staff login endpoint (legacy - with JWT token)"""
    user = authenticate_staff(login_data.username, login_data.password, db)
    print("Authenticated User:", 1)  # Debugging statement
    if user is None:
        return returnException("Incorrect username or password")
    # Get the stored salt and password hash from the user object
    stored_password_hash = user.password
